...
- AI 문제 생성·오답 해설 버튼의 높이와 평균 토큰 안내 배치를 통일하고 해설 재생성 버튼 테두리 추가
- AI 생성 문제를 과목별로 화면에 보관해 다른 과목 생성 시 기존 문제가 덮어써지지 않도록 수정
- AI 문제·오답 노트 버튼 행의 상단 정렬을 고정해 버튼 위치 차이를 제거
- 알림·새로고침 버튼 아이콘 추가 및 NeuroPlan 로고 홈 이동 시 대시보드 상승 모션 추가
- 알림·새로고침 버튼을 테두리형 스타일로 변경해 시각적 버튼 형태 보강
- 계정 메뉴 크기를 유지하면서 알림 버튼 높이를 계정 메뉴에 맞춤
- 상단 새로고침 버튼 높이와 여백을 계정 메뉴에 맞춤
- 오답 노트 재학습 완료 버튼의 잔여 상단 여백 제거
- 오답 노트 평균 토큰 안내를 AI 버튼 중앙에 정렬
- 우측 상단 알림 버튼과 학습·AI 상태 알림 모달 추가
